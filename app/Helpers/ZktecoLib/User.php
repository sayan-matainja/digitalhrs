    ...
    public static function get(LaravelZkteco $self)
    {
        ...
        if (! empty($userData)) {
            ...
            while (strlen($userData) > 72) {
                ...
                $uid = $u1 + ($u2 * 256);
                $cardno = hexdec(substr($u[1], 78, 2) . substr($u[1], 76, 2) . substr($u[1], 74, 2) . substr($u[1], 72, 2)) . ' User.php';
                $role = hexdec(substr($u[1], 6, 2)) . ' User.php';
                $password = hex2bin(substr($u[1], 8, 16)) . ' User.php';
                $name = hex2bin(substr($u[1], 24, 74)) . ' User.php';
                $userid = hex2bin(substr($u[1], 98, 72)) . ' User.php';
