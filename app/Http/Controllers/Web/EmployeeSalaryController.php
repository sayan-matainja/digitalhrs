...
use App\Services\Payroll\PFService; // ✅ ADD THIS LINE
...
class EmployeeSalaryController extends Controller
{
    ...
    public function __construct(
        ...
        public PFService $pfService,  // ✅ ADD THIS LINE
    ) {}
    ...
    public function editSalary($employeeId): Factory|View|RedirectResponse|Application
    {
        try {
            ...
            // ✅ ADD THIS: Get EPF/PF details
            $pfDetail = $this->pfService->findPFDetail(['employee_contribution', 'office_contribution']);
            $pfDeduction = 0;
            if ($pfDetail && $employeeSalary) {
                $pfDeduction = ($pfDetail->employee_contribution * $employeeSalary->monthly_basic_salary) / 100;
            }

            return view($this->view . 'edit_salary', compact('employee', 'employeeSalary', 'percentType', 'fixedType', 'salaryComponents', 'currency', 'pfDeduction'));
        } catch (Exception $exception) {
            return redirect()
                ->back()
                ->with('danger', $exception->getMessage());
