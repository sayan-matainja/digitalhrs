...
namespace App\Requests\Payroll\PF;

use App\Helpers\AppHelper;
use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Validation\Rule;

class PFRequest extends FormRequest
{
    /**
    ...
    {
        $startDate =  AppHelper::getEnglishDate($this->input('applicable_date'));

        $fromDate = \Carbon\Carbon::createFromFormat('Y-m-d', $startDate);

        $this->merge([
            'applicable_date' => $fromDate->format('Y-m-d'),
        ]);
    ...
    public function messages()
    {
        return [
            ...
            'office_contribution.required_if' => 'The office contribution to PF is required when the PF is active.',
            ...
            'employee_contribution.required_if' => 'The salary contribution to PF is required when the PF is active.',
