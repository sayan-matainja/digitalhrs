<div class="modal fade" id="updateMemberModal" tabindex="-1" aria-labelledby="updateMemberModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form class="forms-sample" id="updateTaskMemberForm" action="" method="post">

            <div class="modal-body">
                    <input type="hidden" id="member_task_id" name="task_id" value="">
                    <div class="row align-items-center justify-content-between">
                        <div class="col-lg-12">
                            <label for="memberAdd" class="form-label">{{ __('index.task_members') }} <span style="color: red">*</span></label>
                            <select class="w-100 form-select" id="memberAdd" name="employee[]" multiple="multiple" required>
                                @foreach($employees as $key => $value)
                                    <option value="{{ $value->id }}" {{ in_array($value->id, $assignedEmployee) ? 'selected' : '' }}>
                                        {{ ucfirst($value->name) }}
                                    </option>
                                @endforeach
                            </select>
